...
ZSharp::FZConjugateHandle ZSharp::FZConjugateRegistry_Delegate::Conjugate(const UFunction* signature, TFunctionRef<void(const FZSelfDescriptiveScriptDelegate&)> initialize)
{
	...
	FZSelfDescriptiveScriptDelegate* sdsd = new FZSelfDescriptiveScriptDelegate { signature };
	initialize(*sdsd);
...
{
	const auto mutableUnmanaged = const_cast<FScriptDelegate*>(unmanaged);
	...
	{
		ConjugateMap.Emplace(mutableUnmanaged, { MakeUnique<FZSelfDescriptiveScriptDelegate>(signature, const_cast<FScriptDelegate*>(unmanaged)), false });
...
ZSharp::FZRuntimeTypeHandle ZSharp::FZConjugateRegistry_Delegate::GetManagedType(const UFunction* signature) const
{
	...
	if (!FZReflectionHelper::GetFFieldClassRuntimeTypeLocator(FDelegateProperty::StaticClass(), locator))
	{
		return {};
	}
