...
public readonly struct ReadWriteGameplayTagContainer : ICloneable<GameplayTagContainer>
{
    
    public ReadWriteGameplayTagContainer()
    {
        _backend = GameplayTagContainerBackend_UE.GetFromPool();
        _token = _backend.Token;
    }
    
    public static implicit operator ReadWriteGameplayTagContainer(GameplayTagContainer container) => new(container._PRIVATE_Backend, container._PRIVATE_Token);
    
    public static implicit operator FGameplayTagContainer(ReadWriteGameplayTagContainer container)
    {
    ...
    private ReadWriteGameplayTagContainer(IGameplayTagContainerBackend backend, GameplayTagContainerToken token)
    {
        _backend = backend;
        _token = token;
    }
